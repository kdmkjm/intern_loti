...
$.noConflict();
	jQuery(function(){
		...
		var boardComment = new BoardComment('extBordMorgue.act','board_Idx=${rsSvc_BoardMorgue.board_Idx}&page=${cpage}', 'commentList', 'commentForm');
	...
	function doDelete(num)
	{
		...
		if(del_confirm == true)
		{
			var url = '${pageContext.request.contextPath}/extBordMorgue.act?task=delete&board_Idx='+num;
	...
	<table width="790" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td>
						...
						<td>
							...
							<table width="746" border="0" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td class="t3" width="648" colspan="5">&nbsp;<c:out
											value="${rsSvc_BoardMorgue.title}" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="t3" width="274">&nbsp;<c:out
											value="${rsSvc_BoardMorgue.userNm}" /></td>
									<td class="sBg" width="1"></td>
									<td class="t1 cbg4" width="97">작성일</td>
									<td class="sBg" width="1"></td>
									<td class="t3" width="275">&nbsp;<c:out
											value="${rsSvc_BoardMorgue.udate}" /></td>
								</tr>
								<tr>
									<td colspan="7" class="lbg3"></td>
								</tr>
								<tr>
									...
									<td class="t3" colspan="5">&nbsp;<c:out
											value="${rsSvc_BoardMorgue.counts}" /></td>
								</tr>
								<tr>
									<td colspan="7" class="lbg3"></td>
								</tr>
								<tr>
									...
									<td class="sBg" width="1"></td>
									<td class="t3" width="648" colspan="5">&nbsp;<html:textarea
											property="content" value="${rsSvc_BoardMorgue.content}"
											style="width:98%;" rows="15" readonly="true"></html:textarea></td>
								</tr>
									...
									<tr>
										...
										<td class="t3" width="648" colspan="5">
											<table width="100%" border="0" cellspacing="0" cellpadding="0">
												<c:forEach items="${rsSvc_BoardMorgue.file_IdxList}" var="file" varStatus="i">
													<tr>
														<td><c:set var="filetype" scope="request" value="N" />
															...
															<a href='<html:rewrite page="/extBordMorgue.act?task=downloadAttachment&file_Idx=${file.file_Idx}"/>'>
				...
				<div id="commentList" style="width: ${commentListWidth}px;">
					<c:forEach items="${rsSvc_BoardMorgue.commentList}" var="item">
						<div class="row">
				...
				<table width="750" border="0" cellspacing="0" cellpadding="0"
					...
					<tr valign="bottom">
						<td align="right"><c:if
								test="${isAdmin == 'Y' || isAuth == 'Y'}">
								<img src='<html:rewrite page="/images/board/btn_modify4.gif"/>'
									align="absmiddle" alt="수정"
									onclick='location.href="<html:rewrite page="/extBordMorgue.act?task=modify&board_Idx=${rsSvc_BoardMorgue.board_Idx}"/>"'
									style="cursor: hand" />
								<img src='<html:rewrite page="/images/board/btn_del.gif"/>'
									align="absmiddle" alt="삭제"
									onclick="doDelete(${rsSvc_BoardMorgue.board_Idx});"
									style="cursor: hand" />
		...
		<tr>
			<td>
				<!--검색 -->
				<form method="POST"
					action='<html:rewrite page="/extBordMorgue.act?task=list"/>'>
					<table width="750" border="0" cellspacing="0" cellpadding="0">
							...
							<div id="mainList" style="width: 746px; z-index: 0;">
								<table id="mainTable" width="746" border="0" cellspacing="0"
									...
									<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"
										var="nowDate" />
									<c:forEach var="list" items="${rsSvc_BoardMorgueList}"
										varStatus="i">
										<!-- 열 배경 -->
										<c:if test="${no%2 == 0}">
										...
										<tr class="${lineBg}">
											...
											<td class="sBg" width="1"></td>
											<td class="t3">&nbsp; 
												<c:if test="${rsSvc_BoardMorgue.board_Idx == list.board_Idx}">
													<span style="color: red; font-weight: bold;"><c:out	value="${list.title}" /></span>
												</c:if> 
												<c:if test="${rsSvc_BoardMorgue.board_Idx != list.board_Idx}">
													<a href="extBordMorgue.act?task=info&board_Idx=${list.board_Idx}&page=${cpage}" style="${titleStyle }">
											...
											<td class="sBg" width="1"></td>
											<td class="t1" width="69"><c:if test="${list.fcount > 0}">
												&nbsp;<img src="/images/board/etc.gif"
														style="vertical-align: middle;" alt="첨부파일" title="첨부파일" />
												</c:if></td>
											<td class="sBg" width="1"></td>
				...
				<table width="750" border="0" cellspacing="0" cellpadding="0">
					<tr valign="bottom">
						<td align="right"><c:if test="${isAdmin == 'Y'}">
								<img src='<html:rewrite page="/images/board/btn_write.gif"/>'
									alt="글쓰기" align="absmiddle"
									onclick="location.href='<html:rewrite page="/extBordMorgue.act?task=create"/>'"
									style="cursor: hand" />
								<img src='<html:rewrite page="/images/board/btn_del.gif"/>'
