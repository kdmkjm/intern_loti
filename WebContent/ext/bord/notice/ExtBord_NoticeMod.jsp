...
$.noConflict();
	jQuery(function(){
		...
		var boardFile = new BoardFile('extBordNotice.act','div_files', 'file_list', 'ProgressLayer');
	});
</script>
<body>
	<html:form method="post" action="/extBordNotice.act?task=insert"
		enctype="multipart/form-data">
		<html:hidden property="board_Idx" />
		<html:hidden property="task" value="insert" />
								...
								<table width="746" border="0" cellspacing="0" cellpadding="0">
									...
									<tr>
										...
										<td class="t3" width="648">&nbsp;<input type="checkbox"
											...
											<c:if test="${rsSvc_BoardNotice.isnotice == 'Y'}">checked</c:if>/>공지사항
										</td>
									</tr>
									<tr>
										<td colspan="3" class="lbg3"></td>
									...
									<tr>
										...
										<td class="t3" width="648"><textarea id="content"
												...
												style="width: 98%; height: 100%; display: none;">${rsSvc_BoardNotice.content}</textarea>
									...
									<tr>
										...
										<td class="sBg" width="1"></td>
										<td class="t3" width="648" valign="top">
													...
													<li>
														<select id="file_list" multiple="multiple" style="width: 300px; height: 100px;">
															<c:if test="${rsSvc_BoardNotice.file_IdxList != null}">
																<c:forEach items="${rsSvc_BoardNotice.file_IdxList}" var="file">
					...
					<table width="750" border="0" cellspacing="0" cellpadding="0"
						...
						<tr valign="bottom">
							<td align="right"><c:if test="${task == 'create'}">
									<img id="SubmitBtn"
										src='<html:rewrite page="/images/board/btn_up3.gif"/>'
										align="absmiddle" alt="등록" style="cursor: hand" />
									<img src='<html:rewrite page="/images/board/btn_cancle2.gif"/>'
										align="absmiddle" alt="취소"
										onclick='location.href="<html:rewrite page="/extBordNotice.act?task=list"/>"'
										style="cursor: hand" />
								</c:if> <c:if test="${task == 'modify'}">
									<img id="ModifyBtn"
										src='<html:rewrite page="/images/board/btn_save.gif"/>'
										align="absmiddle" alt="저장" style="cursor: hand" />
									<img src='<html:rewrite page="/images/board/btn_cancle2.gif"/>'
										align="absmiddle" alt="취소"
										onclick='location.href="<html:rewrite page="/extBordNotice.act?task=info&board_Idx=${rsSvc_BoardNotice.board_Idx}"/>"'
										style="cursor: hand" />
								</c:if></td>
						</tr>
					</table> <!--하단버튼 테이블 끝-->
