...
$.noConflict();
	...
	function checkDelete(){
		if(jQuery('#mainTable input[name="board_Idx_array"]:checked').size() == 0){
			...
			if(confirm('삭제하시겠습니까?')){
				var $form = jQuery('<form action="/extBordNotice.act?task=delete" method="post"/>').appendTo('body');
						...
						<div id="mainList" style="width: 746px; z-index: 0;">
							<table id="mainTable" width="746" border="0" cellspacing="0"
								...
								<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"
									var="nowDate" />
								<c:forEach var="list" items="${rsSvc_BoardNoticeList}"
									varStatus="i">
									<!-- 열 배경 -->
									<c:if test="${no%2 == 0}">
				...
				<tr valign="bottom">
					<td align="right"><c:if test="${isAdmin == 'Y'}">
							<img src='<html:rewrite page="/images/board/btn_write.gif"/>'
								...
								onclick="location.href='<html:rewrite page="/extBordNotice.act?task=create"/>'"
								style="cursor: hand" />
