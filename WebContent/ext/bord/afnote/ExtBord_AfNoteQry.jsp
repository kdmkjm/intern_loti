...
$.noConflict();
	function doDelete(num)
	{
		...
		if(del_confirm == true)
		{
			var url = '${pageContext.request.contextPath}/extBordAfNote.act?task=delete&board_Idx='+num;
	...
	jQuery(function(){
		...
		var boardComment = new BoardComment('extBordAfNote.act','board_Idx=${rsSvc_BoardAfNote.board_Idx}&page=${cpage}', 'commentList', 'commentForm');
	...
	<table width="790" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td>
							...
							<table width="746" border="0" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td class="t3" width="648" colspan="5">&nbsp;<c:out
											value="${rsSvc_BoardAfNote.title}" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="t3" width="274">&nbsp;<c:out
											value="${rsSvc_BoardAfNote.userNm}" /></td>
									<td class="sBg" width="1"></td>
									<td class="t1 cbg4" width="97">작성일</td>
									<td class="sBg" width="1"></td>
									<td class="t3" width="275">&nbsp;<c:out
											value="${rsSvc_BoardAfNote.udate}" /></td>
								</tr>
								<tr>
									<td colspan="7" class="lbg3"></td>
								</tr>
								<tr>
									...
									<td class="t3" colspan="5">&nbsp;<c:out
											value="${rsSvc_BoardAfNote.counts}" /></td>
								</tr>
								<tr>
									<td colspan="7" class="lbg3"></td>
								</tr>
								<tr>
									...
									<td class="t3" width="648" colspan="5">&nbsp;<html:textarea
											property="content" value="${rsSvc_BoardAfNote.content}"
											style="width:98%;" rows="18" readonly="true"></html:textarea></td>
								</tr>
							</table> <!--리스트 끝-->
						</td>
					</tr>
				...
				<div id="commentList" style="width: ${commentListWidth}px;">
					<c:forEach items="${rsSvc_BoardAfNote.commentList}" var="item">
						<div class="row">
					...
					<tr valign="bottom">
						<td align="right"><c:if
								...
								<img src='<html:rewrite page="/images/board/btn_modify4.gif"/>'
									...
									onclick='location.href="<html:rewrite page="/extBordAfNote.act?task=modify&board_Idx=${rsSvc_BoardAfNote.board_Idx}"/>"'
									style="cursor: hand" />
								<img src='<html:rewrite page="/images/board/btn_del.gif"/>'
									align="absmiddle" alt="삭제"
									onclick="doDelete(${rsSvc_BoardAfNote.board_Idx});"
									style="cursor: hand" />
		...
		<tr>
			<td>
				<!--검색 -->
				<form method="POST"
					action='<html:rewrite page="/extBordAfNote.act?task=list"/>'>
					<table width="750" border="0" cellspacing="0" cellpadding="0">
							...
							<div id="mainList" style="width: 746px; z-index: 0;">
								<table id="mainTable" width="746" border="0" cellspacing="0"
									...
									<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"
										var="nowDate" />
									<c:forEach var="list" items="${rsSvc_BoardAfNoteList}"
										varStatus="i">
										<!-- 열 배경 -->
										<c:if test="${no%2 == 0}">
										...
										<tr class="${lineBg}">
											...
											<td class="sBg" width="1"></td>
											<td class="t3">&nbsp; 
												<c:if test="${rsSvc_BoardAfNote.board_Idx == list.board_Idx}">
													<span style="color: red; font-weight: bold;"><c:out	value="${list.title}" /></span>
												</c:if>
												<c:if test="${rsSvc_BoardAfNote.board_Idx != list.board_Idx}">
													<a href="extBordAfNote.act?task=info&board_Idx=${list.board_Idx}&page=${cpage}" style="${titleStyle }">
				...
				<table width="750" border="0" cellspacing="0" cellpadding="0">
					<tr valign="bottom">
						<td align="right"><img
							src='<html:rewrite page="/images/board/btn_write.gif"/>' alt="글쓰기"
							align="absmiddle"
							onclick="location.href='<html:rewrite page="/extBordAfNote.act?task=create"/>'"
							style="cursor: hand" />
							<c:if test="${isAdmin == 'Y'}">
								<img src='<html:rewrite page="/images/board/btn_del.gif"/>'
