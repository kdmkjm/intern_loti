...
<script language="JavaScript">
	...
	function passList(){
		...
		window.open('<html:rewrite page="/oprPass.act?task=passlist&searchCourseCode=' + courseCode +'"/>', '수료자명단', 'scrollbars=no, menubar=no, width=50, height=50, status=no top=150, left=150');
	}
//-->
</script>
...
<tr>
  <td>
	...
	<h1><img alt="이수관리" src="/images/operation/title_34.gif" /></h1>
	</div>
  </td>
</tr>
<tr>
  <td>
	<!--검색 -->
	<html:form method="POST" action="oprPass.act?task=list">
	...
	<html:form method="POST" action="oprPass.act?task=pass">
	  ...
	  <tr valign="bottom">
		<td>
		  <c:if test="${costCalc == '0'}">
			<img src='<html:rewrite page="/images/board/btn_completion2.gif"/>' align="absmiddle" alt="이수처리" style="cursor:hand" onclick="doSubmit(document.forms[1]);"/>
		  </c:if> 
		  <c:if test="${costCalc != '0'}">
			<img src='<html:rewrite page="/images/board/btn_completion2_off.gif"/>' align="absmiddle"/>
		  </c:if> 
		</td>
	  </tr>
	  </table>
