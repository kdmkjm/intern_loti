...
function Calendar_D(ctl, ctl2, format) {
		//alert(ctl2);
	var leftpos = 0;
	var toppos = 0;
	...
	if(bPageLoaded) {
		...
		aFormat	= dateFormat.split(formatChar);
			...
			cleft =	fixedX==-1 ? ctl.offsetLeft	+ leftpos - 100 :	fixedX;
						
			if((window.screen.availHeight - getAbsoluteTop(ctl)) < 300)
				ctop = getAbsoluteTop(ctl) - 178;
			else
