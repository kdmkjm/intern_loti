	...
	// Node object
	function Node(id, pid, level, name, check, type, menuNo ,url, title, target, icon, iconOpen, open) {
		this.id = id;
		this.pid = pid;
		this.level = level;
		this.name = name;
	...
	dTree.prototype.add = function(id, pid, level, name, check, type, menuNo,url , title, target, icon, iconOpen, open) {
		this.aNodes[this.aNodes.length] = new Node(id, pid, level, name, check, type, menuNo, url, title, target, icon, iconOpen, open);
	};
	...
	dTree.prototype.toString = function() {
		var str = '<div class="dtree" style="overflow-x:hidden;overflow-y:scroll;height:250px;">\n';
	...
	dTree.prototype.node = function(node, nodeId) {
		...
		if (this.config.useIcons) {
			...
			*/
			...
			if(node.type=='F'){
			 	str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
				str += '<input type="checkbox"" name="select" id="i' + this.obj + nodeId + '" LVL="'+node.pid+'" level ="'+node.level+'" menutype="'+node.type+'" value="'+node.menuNo+'" onclick=igroup_t_mst_chk(this) />';
				str += '<input type="hidden" id="isCheck" name="isCheck" value="'+node.check+'"/>';
				str += '<input type="hidden" id="insMenuNo" name="insMenuNo" value="'+node.menuNo+'"/>';
...
//트리메뉴에서 체크박스 체크되게끔 수
function igroup_t_mst_chk(obj)
{
  var objs = document.forms[1].getElementsByTagName("input");
  
	if(obj.attributes["level"].value == "1"){
 		 for(var iCount1=0; iCount1 < objs.length; iCount1++) {
    		tobj1 = objs[iCount1];
    		tobjname1 = tobj1.name.substring(0,9);
    		if ( tobjname1!="select" ) continue;
    		tobj1.checked = obj.checked;
 		 }
 		 
	}else if(obj.attributes["level"].value == "2"){
 		 for(var iCount2_1=0; iCount2_1 < objs.length; iCount2_1++) {
    		tobj2_1 = objs[iCount2_1];
    		tobjname2_1 = tobj2_1.name.substring(0,9);
    		if ( tobjname2_1 != "select" ) continue;
    		if(obj.value == tobj2_1.attributes["LVL"].value) {
	 					tobj2_1.checked = obj.checked;
	 					
	 					 for(var iCount2_2=0; iCount2_2 < objs.length; iCount2_2++) {
				    		tobj2_2 = objs[iCount2_2];
				    		tobjname2_2 = tobj2_2.name.substring(0,9);
				    		if ( tobjname2_2 != "select" ) continue;
				    		if(tobj2_1.value == tobj2_2.attributes["LVL"].value) {
					 					tobj2_2.checked = tobj2_1.checked;
					 			}
				 		 }
	 			}
 		 }
	}else if(obj.attributes["level"].value == "3"){
 		 for(var iCount3=0; iCount3 < objs.length; iCount3++) {
    		tobj3 = objs[iCount3];
    		tobjname3 = tobj3.name.substring(0,9);
    		if ( tobjname3 != "select" ) continue;
    		if(obj.value == tobj3.attributes["LVL"].value) {
	 					tobj3.checked = obj.checked;
	 			}
 		 }
	}
}
