...
<c:forEach var="survList" items="${rsPln_EduPlanList}" varStatus="i" ><c:if test="${i.count%2 == 0}"><c:set var="lineBg" scope="request" value="tbg3"/></c:if><c:if test="${i.count%2 == 1}"><c:set var="lineBg" scope="request" value=""/></c:if>																	
  <tr class="${lineBg}">
		...
		<td class="t2Line" width="50">
			...
			<c:if test="${isFix > 0}"><img src='<html:rewrite page="/images/board/btn_modify_off.gif"/>' width="34" height="18" align="absmiddle" alt="수정"  /></c:if>
		</td>
	</tr>
    ...
    <tr valign="bottom">
      <td>
        ...
        <c:if test="${isFix > 0}">
          ...
          <img src='<html:rewrite page="/images/board/btn_new_up_off.gif"/>' alt="집합과정 신규등록" align="absmiddle"  />
          <img src='<html:rewrite page="/images/board/btn_new_cyberup_off.gif"/>' alt="사이버과정 신규등록" align="absmiddle"  />
        </c:if>
      </td>
      <td align="right">
